...
- `FeatureLayer.update_attachment()` replaces the file of a single existing attachment on one feature; `FeatureLayer.update_attachments()` accepts flat parallel `object_ids`, `attachment_ids`, and `files` iterables and fans out concurrent `updateAttachment` uploads. Both accept `Path`, open binary file objects, or raw `bytes`, with the same filename/MIME handling as the add methods. Both guard on `supports_attachments()` and the new `supports_update()`, raising `LayerCapabilityError` when the layer does not support attachments or updating.
- `FeatureLayer.supports_update()` reports whether the layer's `capabilities` include the `Update` operation.
- `FeatureLayer.delete_attachment()` deletes a single attachment from one feature; `FeatureLayer.delete_attachments()` accepts flat parallel `object_ids` and `attachment_ids` iterables, groups them by OBJECTID internally, and fires one concurrent `deleteAttachments` request per unique feature. Result order matches input pair order. Both guard on `supports_attachments()` and raise `LayerCapabilityError` when the layer does not support attachments.
...
- `AddAttachmentsResult` renamed to `AttachmentsResult` and `add_attachments.py` consolidated into `attachments.py` to share a module with `DeleteAttachmentsOperation`. The result type is now returned by both add and delete methods.
